...
export class GuildsService extends BaseService {
	 ...
	 */
	guildsControllerGuilds$Response(
		params?: {},
		context?: HttpContext
	): Observable<StrictHttpResponse<Array<BotOAuth2GuildDto>>> {
		const rb = new RequestBuilder(
			this.rootUrl,
			GuildsService.GuildsControllerGuildsPath,
			'get'
		);
		if (params) {
		}
	...
	guildsControllerGuilds(
		...
		return this.guildsControllerGuilds$Response(params, context).pipe(
			map(
				(r: StrictHttpResponse<Array<BotOAuth2GuildDto>>) =>
					r.body as Array<BotOAuth2GuildDto>
			)
		);
	}
	 ...
	 * This method doesn't expect any request body.
	 */
	guildsControllerTest$Response(
		params?: {},
		context?: HttpContext
	): Observable<StrictHttpResponse<Array<UsersEntity>>> {
		const rb = new RequestBuilder(
			this.rootUrl,
			GuildsService.GuildsControllerTestPath,
			'get'
		);
		if (params) {
		}
